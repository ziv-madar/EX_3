...
def test_memory(prog,input_f):
    global passed
    try:
        rc = run_cmd("valgrind --leak-check=full --error-exitcode=1 ./%s < %s > /dev/null"%(prog,input_f))
        if rc < 0:
            passed = 0
            print("FAILED valgrind memory check")
        return
    except Exception as inst:
...
def main():
    ...
    test_output_vs_expected("isort" , "../inputs/sort_input.txt" , "../outputs/sort_output.txt")
    test_output_vs_expected("txtfind" , "../inputs/find_inputa.txt" , "../outputs/find_outputa.txt")
    test_output_vs_expected("txtfind" , "../inputs/find_inputb.txt" , "../outputs/find_outputb.txt")
    
    test_memory("txtfind" , "../inputs/find_inputa.txt")
    test_memory("isort" , "../inputs/sort_input.txt")
    if passed == 1:
